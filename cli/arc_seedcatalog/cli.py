...
def build(path,seed,epoch):
    doc=json.loads(Path(path).read_text()); receipts=[]; rh=sha("arc.seedcatalog.ruleset.v8"); cmh=sha("category-map-v8"); ah=sha("adapter-v8")
    for it in rows(doc):
        ...
        core={"schema":"arc.seedcatalog.entry_receipt.v8","entry_id":"hmac-sha256:"+hm(seed,sid+"|"+vol),"source_id":"sha256:"+sid,"category_vector":"hmac-sha256:"+hm(seed,c+"|"+rh+"|"+cmh),"category_path_hash":"sha256:"+sha(c),"ruleset_hash":"sha256:"+rh,"category_map_hash":"sha256:"+cmh,"adapter_profile_hash":"sha256:"+ah,"epoch":epoch,"stored_raw_data":False,"stores_title":False,"stores_path":False,"stores_server_name":False,"stores_url":False,"stores_user_data":False}
        core["receipt_hash"]="sha256:"+sha(canon(core)); receipts.append(core)
    receipts.sort(key=lambda r:r["receipt_hash"])
    rb={"schema":"arc.seedcatalog.receipt_bundle.v8","count":len(receipts),"receipts":receipts,"stores_raw_data":False}; rb["bundle_hash"]="sha256:"+sha(canon(rb))
    records=[{"schema":"arc.core.seedcatalog_registration.v8","source":"arc-seedcatalog","object_type":"seedcatalog_entry_receipt","receipt_hash":r["receipt_hash"],"entry_id":r["entry_id"],"source_id":r["source_id"],"category_vector":r["category_vector"],"category_path_hash":r["category_path_hash"],"ruleset_hash":r["ruleset_hash"],"category_map_hash":r["category_map_hash"],"adapter_profile_hash":r["adapter_profile_hash"],"stores_raw_data":False} for r in receipts]
    ac={"schema":"arc.core.seedcatalog_handoff_bundle.v8","records":records,"entry_count":len(records),"stores_raw_data":False}; ac["bundle_hash"]="sha256:"+sha(canon(ac))
    pol={"schema":"arc.seedcatalog.policy_bundle.v8","raw_field_denylist":sorted(DENY),"stores_raw_data":False}; pol["bundle_hash"]="sha256:"+sha(canon(pol))
    sig={"schema":"arc.seedcatalog.signature_envelope.v8","status":"unsigned","signing_algorithm":"ed25519-planned","signable_hashes":{"receipt_bundle":rb["bundle_hash"],"arc_core_handoff_bundle":ac["bundle_hash"],"policy_bundle":pol["bundle_hash"]},"stores_raw_data":False}; sig["bundle_hash"]="sha256:"+sha(canon(sig))
    split={"schema":"arc.seedcatalog.split_bundle.v8","receipt_bundle":rb,"policy_bundle":pol,"arc_core_handoff_bundle":ac,"signature_envelope":sig,"stores_raw_data":False}; split["catalog_hash"]="sha256:"+sha(canon(split)); return split
...
def proof_pack(path,out):
    data=json.loads(Path(path).read_text()); pack={"schema":"arc.seedcatalog.proof_pack.v8","split_bundle":data,"stores_raw_data":False}; Path(out).parent.mkdir(parents=True,exist_ok=True); Path(out).write_text(json.dumps(pack,indent=2)+"\n"); print(out)
...
def export_binary(path,out):
    data=json.loads(Path(path).read_text()); payload=canon(data).encode(); blob=b"ARCSCAT\0"+struct.pack(">I",8)+struct.pack(">Q",len(payload))+hashlib.sha256(payload).digest()+payload; Path(out).parent.mkdir(parents=True,exist_ok=True); Path(out).write_bytes(blob); print(out)
def verify_binary(path):
    b=Path(path).read_bytes(); ok=b[:8]==b"ARCSCAT\0"; ver=struct.unpack(">I",b[8:12])[0]; ln=struct.unpack(">Q",b[12:20])[0]; dg=b[20:52]; payload=b[52:]; ok=ok and ver==8 and ln==len(payload) and hashlib.sha256(payload).digest()==dg; print(json.dumps({"ok":ok,"version":ver,"payload_length":len(payload),"payload_sha256":dg.hex()},indent=2)); return 0 if ok else 1
def main():
